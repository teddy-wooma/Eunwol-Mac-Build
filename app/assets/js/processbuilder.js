...
class ProcessBuilder {
    ...
    build(){
        ...
        // 🛡️ [은월마을 특각 조치] 마인크래프트 게임 실행 0.1초 전, options.txt 리소스팩 라인 강제 주입
                try {
                            const optionsPath = path.join(this.gameDir, 'options.txt')
                            const targetLine = 'resourcePacks:["vanilla","file/EunwolVillagePack.zip"]'

                            if (fs.existsSync(optionsPath)) {
                                let data = fs.readFileSync(optionsPath, 'utf8')

                                if (data.includes('resourcePacks:')) {
                                    // 정규식을 사용해 기존 resourcePacks:[...] 라인을 통째로 낚아채서 지정한 최종 형태로 치환합니다.
                                    data = data.replace(/resourcePacks:\[.*\]/g, targetLine)
                                } else {
                                    // 파일은 있는데 혹시나 resourcePacks 구문이 통째로 누락되어 있다면 맨 밑에 추가
                                    data += `\n${targetLine}\n`
                                }

                                fs.writeFileSync(optionsPath, data, 'utf8')
                                logger.info('[은월마을] options.txt 리소스팩 라인을 지정된 팩 목록으로 강제 셋업 완료!')
                            } else {
                                // 최초 실행이라 options.txt가 아예 없는 경우 새 규격으로 빌드
                                const defaultOptions = `${targetLine}\n`
                                fs.writeFileSync(optionsPath, defaultOptions, 'utf8')
                                logger.info('[은월마을] options.txt 기본 파일 생성 및 리소스팩 강제 세팅 완료.')
                            }
                        } catch (err) {
                            logger.error('[은월마을] 마크 실행 전 리소스팩 강제 변환 실패:', err)
                        }
