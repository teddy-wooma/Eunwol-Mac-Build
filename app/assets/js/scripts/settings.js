...
function populateReleaseNotes(){
    $.ajax({
        url: 'https://github.com/teddy-wooma/luna-eunwol-launcher/releases.atom',
        success: (data) => {
            const entries = $(data).find('entry')

            if (entries.length > 0) {
                // 깃허브 피드에서 가장 위에 있는 최신 글(v1.0.1)을 무조건 가져옵니다.
                const latestEntry = $(entries[0])

                // 런처 설정창에 깃허브에서 긁어온 제목과 본문을 강제로 채워넣습니다.
                settingsAboutChangelogTitle.innerHTML = "새로운 업데이트 발견: " + latestEntry.find('title').text()
...
function prepareUpdateTab(data = null){
    // 만약 메인 프로세스로부터 받은 데이터가 없다면, [정보] 탭처럼 여기서 직접 깃허브를 비동기 조회합니다.
    if (data == null) {
        $.ajax({
            url: 'https://github.com/teddy-wooma/luna-eunwol-launcher/releases.atom',
            success: (feedData) => {
                const entries = $(feedData).find('entry')
                if (entries.length > 0) {
                    let latestTag = $(entries[0]).find('id').text()
                    latestTag = latestTag.substring(latestTag.lastIndexOf('/') + 1) // 예: "v1.0.1"
                    const currentVersion = 'v' + remote.app.getVersion() // 예: "v1.0.0"

                    if (latestTag !== currentVersion) {
                        // ❌ 버전이 다르면 가상 데이터 세트를 만들어 상단 if문으로 강제 토스!
                        const mockData = {
                            version: latestTag.replace('v', ''),
                            releaseName: $(entries[0]).find('title').text(),
                            releaseNotes: $(entries[0]).find('content').text(),
                            darwindownload: $(entries[0]).find('link').attr('href')
                        }
                        populateSettingsUpdateInformation(mockData)
                    } else {
